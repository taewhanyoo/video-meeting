...
const MainVideoPage = ()=>{
    ...
    useEffect( ()=>{
        ...
        console.log(token);
        const fetchDecodedToken = async ()=>{
            // const resp = await axios.post('https://192.168.55.8:9000/validate-link', {token});
            const resp = await axios.post('https://www.yoo.homes/validate-link', {token});
            console.log(resp.data);
            setApptInfo(resp.data);
            uuidRef.current = resp.data.uuid;  // added by taewhan
        }
        fetchDecodedToken();
    },[])
